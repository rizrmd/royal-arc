...
export const jsx = (...args: any[]) => {
  ...
  const res = modifyProps(...args)
  return w._jsx(...res.args)
}

export const jsxs = (...args: any[]) => {
  const w = window as any
  const res = modifyProps(...args)
  return w._jsxs(...res.args)
}

const modifyProps = (...args: any[]) => {
  const w = window as any
  ...
  const newargs = propsModified ? [tag, props, ...args.slice(2)] : args
  return { args: newargs }
}
