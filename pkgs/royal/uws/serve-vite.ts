...
import { SrvHttpResponse, SSROverride } from '../export'
...
export const serveVite = async (
  ...
  pathname: string,
  res: SrvHttpResponse,
  override?: (ovr: SSROverride, res: SrvHttpResponse) => Promise<void>
) => {
        ...
        res,
        {
          async override(r) {
            if (r.headers['content-type'] === 'text/html') {
              const body =
                typeof r.body === 'string' ? r.body : dec.decode(r.body)
              r.body = replaceBodyDev(body, conf[webName].url, srvurl)
            }

            if (override) {
              const _url = new URL(r.url)
              r.url = _url.pathname

              await override(r, res)
            }
          },
        }
      ))
